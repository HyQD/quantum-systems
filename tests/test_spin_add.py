...
from quantum_systems import SpinOrbitalSystem
from quantum_systems.system_helper import add_spin_spf, add_spin_one_body
...
def test_get_spin_block_slices():
    ...
    o_a, o_b, v_a, v_b = SpinOrbitalSystem.get_spin_block_slices(n, n_a, l)
    ...
    np.testing.assert_allclose(ind[o_a], np.arange(0, 2 * n_a, 2))

    np.testing.assert_allclose(ind[o_b], np.arange(1, 2 * (n - n_a), 2))

    np.testing.assert_allclose(ind[v_a], np.arange(2 * n_a, l, 2))

    np.testing.assert_allclose(ind[v_b], np.arange(2 * (n - n_a) + 1, l, 2))
...
def test_add_spin_one_body():
    ...
    for n_a in range(0, n):
        ...
        np.testing.assert_allclose(new_h[o_a, o_a], h[:n_a, :n_a])

        np.testing.assert_allclose(new_h[o_b, o_b], h[:n_b, :n_b])

        np.testing.assert_allclose(new_h[v_a, v_a], h[n_a:, n_a:])

        np.testing.assert_allclose(new_h[v_b, v_b], h[n_b:, n_b:])
...
def test_restricted_odho():
    ...
    np.testing.assert_allclose(odho.spf[odho.o_a], odho.spf[odho.o_b])

    np.testing.assert_allclose(odho.spf[odho.v_a], odho.spf[odho.v_b])
    ...
    np.testing.assert_allclose(odho_res.spf[o_res], odho.spf[odho.o_a])

    np.testing.assert_allclose(odho_res.spf[v_res], odho.spf[odho.v_a])
...
def test_open_odqd():
    ...
    np.testing.assert_allclose(odho_res.spf[:n_a], odho.spf[odho.o_a])

    np.testing.assert_allclose(odho_res.spf[: n - n_a], odho.spf[odho.o_b])
